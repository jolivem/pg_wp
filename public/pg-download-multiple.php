...
class Pg_Download_Multiple_Public {
    ...
    // @brief Add image to gallery, check if already exists before
    ...
    function add_image_to_gallery($gallery_id, $image_id){
        ...
        if( isset($image_id) && $image_id != '' && isset($gallery_id) && $gallery_id != '') {
            ...
            //error_log("add_image_to_gallery current liste result=".print_r($result, true));
            //error_log("add_image_to_gallery images_id=".$result[0]["images_ids"]);
            $tab_ids = explode( "***", $result[0]["images_ids"]);
            //error_log("add_image_to_gallery current liste tab_ids=".print_r($tab_ids, true));
            if ($tab_ids != null ) {
                // find if exists
                $in = in_array($image_id, $tab_ids);
                //error_log("add_image_to_gallery in=".$in);
                if ($in == true) {
                    // no ids
                    $wpdb->query('ROLLBACK');
